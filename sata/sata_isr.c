...
static UINT32 initq = 1;
extern Queue queue;

static __inline void softwareq_push(Queue *queue, UINT32 cmd_type, UINT32 lba, UINT32 sector_count){ //linked list

	Node *node = (Node*)malloc(sizeof(Node));
	node->data->lba = lba;
	node->data->sector_count = sector_count;
	node->data->cmd_type = cmd_type;
	node->next = NULL;
	
	if (queue-> count == 0){ //when queue is empty
		queue->front = node; //set node as front
	}
	else{ //when queue is not empty
		queue->rear->next = node; //set rear's next as current node
	}
	queue->rear->next = node; //rear as current node
	queue->count++; //increase count

	uart_printf("pushed lba: %d\n", node->data->lba);
	uart_printf("pushed sector_count: %d\n", node->data->sector_count);
	uart_printf("pushed cmd_type: %d\n", node->data->cmd_type);
	uart_printf("count: %d\n", queue->count);
}
...
static __inline void handle_got_cfis(void) //cfis: command fis
{
	...
	fis_d1 = GETREG(SATA_FIS_H2D_1); //H2D: HBA(Host Bus Adapter) to Device
	fis_d3 = GETREG(SATA_FIS_H2D_3);
	//HBA: silicon that implements the AHCI specification to communicate beteween system memory and SATA devices
	//support queing

	if (cmd_type & ATR_LBA_NOR) //cmd_type == 0x00 -> NOP: no operation,  0x01 (?)
	{
		if ((fis_d1 & BIT30) == 0)	// CHS: Cylinder Head Sector
		{
	...
	//lba, sector_count, cmd_type loaded up to here

	if(initq == 1) //initialize queue
	{
	...
	softwareq_push(&queue, cmd_type, lba, sector_count); //enqueue

	if (lba + sector_count > MAX_LBA + 1 && (cmd_type & ATR_NO_SECT) == 0)
	{
		send_status_to_host(B_IDNF);
	}
	else if (cmd_type & (CCL_FTL_H2D | CCL_FTL_D2H)) //write or read -> both read/write != 0, there exist at list one write/read
	{
		UINT32 action_flags;
		...
		if (cmd_type & CCL_FTL_H2D) //if write
		{
		...
		else //if read
		{
			...
			if (cmd_code == ATA_READ_DMA || cmd_code == ATA_READ_DMA_EXT) //if read DMA or read DMA exit
			{
				action_flags = DMA_READ | COMPLETE;
			}
			else
			{
				UINT32 fis_type = FISTYPE_PIO_SETUP; //PIO: programmed Input/Output -> data transmission via IO between CPU & peripheral device
				// ATA, SATA uses PIO
				UINT32 flags = B_IRQ;
				UINT32 status = B_DRDY | BIT4 | B_DRQ; //DRQ: data transfer requested
				UINT32 e_status = B_DRDY | BIT4; //hbaPioESts set to the E_status field of the PIO Setup FIS until the data
				//for the DRQ block is transferred. On power-up/reset of the HBA port, hbaPioEsts cleared to 0
		...
		if (GETREG(SATA_EQ_STATUS) >> 31) //eventq full (?)
		{
