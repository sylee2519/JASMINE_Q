...
static void eventq_get(CMD_T* cmd)
{
	...
	cmd->sector_count	= EQReadData0 >> 16; //some kind of operation(?) fall of 16 bits
...
void softwareq_pop(Queue *queue, CMD_T* pop){ //linked list

	Node *node = queue->front; //first node as node
	
	//return values
	pop->cmd_type = node->data->cmd_type;
	pop->lba = node->data->lba;
	pop->sector_count = node->data-> sector_count;

	queue->front = node->next; //next one as node
	free(node); //free first node
	queue->count--; //decrease count

	//for check
	uart_printf("Popped lba: %d\n", pop->lba);
	uart_printf("Popped sector_count: %d\n", pop->sector_count);
	uart_printf("Popped cmd_type: %d\n", pop->cmd_type);
	uart_printf("count: %d\n", queue->count);
	...
	while (1)
	{
		if (eventq_get_count()) 
		{
			...
			if(!IsEmpty(&queue)){
				...
				softwareq_pop(&queue, &pop);
			
				if (pop.cmd_type == READ)
				{
				ftl_read(pop.lba, pop.sector_count);
				}

				else
				{
				ftl_write(pop.lba, pop.sector_count);
				}
